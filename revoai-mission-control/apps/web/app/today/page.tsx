              ...
              <>
                {autopilotDrafts.length > 0 && (
                  <Card title="⚡ Autopilot drafts ready for your review" subtitle="Auto-generated by your daily autopilot. Approve to schedule, reject to discard.">
                    {autopilotDrafts.length > 1 && (
                      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 10 }}>
                        <Button variant="primary" onClick={async () => {
                          if (!confirm(`Approve & schedule all ${autopilotDrafts.length} autopilot drafts? Each fires at its own best-time window.`)) return;
                          let ok = 0, failed = 0;
                          for (const p of autopilotDrafts) {
                            try {
                              const r = await fetch(`${API_BASE}/api/social-posts/${p.id}/approve`, { method: 'POST', credentials: 'include', headers: apiHeaders });
                              if (r.ok) ok++; else failed++;
                              await new Promise((res) => setTimeout(res, 200));
                            } catch { failed++; }
                          }
                          toast(failed === 0 ? 'success' : 'warning', `Approved ${ok}${failed ? ` · ${failed} failed` : ''}`);
                          load();
                        }}>✓ Approve all {autopilotDrafts.length}</Button>
                      </div>
                    )}
                    <div style={{ display: 'grid', gap: 8 }}>
